...
(function (w) {
  ...
  // ---- 職業特化渲染（只有有值才顯示）----
  function renderJobSpecialSection(baseJob, total, p) {
    const _pct = (x, d=1) => (Number(x||0)*100).toFixed(d) + "%";
    const _fmt = (nv) => Number(nv||0).toLocaleString();

    if (baseJob === "archer") {
      if (!total.preemptiveEnabled) return "";
      const chance = Number(total.preemptiveChance)||0;
      const max = Math.max(0, Number(total.preemptivePerAttackMax)||0);
      if (chance<=0 && max<=0) return "";
      return `
        <div class="section">
          <div class="section-title"><span class="chip chip-teal">🏹 職業特化：弓箭手</span></div>
          <div class="grid-2">
            <div class="tile"><div class="tile-label">先手再動</div><div class="tile-value">${_pct(chance)}</div></div>
            <div class="tile"><div class="tile-label">再動次數上限</div><div class="tile-value">${max}</div></div>
          </div>
        </div>
      `;
    }
    ...
    if (baseJob === "thief") {
      const combo = Number(total.comboRate)||0;
      const dbl   = Number(total.doubleHitChance)||0;
      if (combo<=0 && dbl<=0) return "";
      return `
        <div class="section">
          <div class="section-title"><span class="chip chip-teal">🗡️ 職業特化：盜賊</span></div>
          <div class="grid-2">
            <div class="tile"><div class="tile-label">連擊率（=雙擊率）</div><div class="tile-value">${_pct(combo)}</div></div>
            <div class="tile"><div class="tile-label">雙擊率</div><div class="tile-value">${_pct(dbl)}</div></div>
          </div>
        </div>
      `;
    }
    ...
    if (baseJob === "mage") {
      const ms = (typeof w.getMagicShieldPercent === "function") ? w.getMagicShieldPercent() : 0;
      if (!p.magicShieldEnabled && ms<=0) return "";
      return `
        <div class="section">
          <div class="section-title"><span class="chip chip-teal">🪄 職業特化：法師</span></div>
          <div class="grid-2">
            <div class="tile"><div class="tile-label">魔力護盾</div><div class="tile-value">${_pct(ms)} ${p.magicShieldEnabled? "" : "<span class='muted'>(關)</span>"}</div></div>
            <div class="tile"><div class="tile-label">INT</div><div class="tile-value">${_fmt((p.baseStats.int||0)+(p.coreBonus.int||0))}</div></div>
          </div>
        </div>
      `;
    }
    ...
    if (baseJob === "warrior") {
      const dr = Number(total.damageReduce)||0;
      if (dr<=0) return "";
      return `
        <div class="section">
          <div class="section-title"><span class="chip chip-teal">🛡️ 職業特化：戰士</span></div>
          <div class="grid-2">
            <div class="tile"><div class="tile-label">總減傷</div><div class="tile-value">${_pct(dr,2)}</div></div>
            <div class="tile"><div class="tile-label">力量（參考）</div><div class="tile-value">${_fmt((p.baseStats.str||0)+(p.coreBonus.str||0))}</div></div>
          </div>
        </div>
      `;
    }
  ...
  function openStatModal() {
    ...
    // 職業特化區塊
    const jobSpecialHtml = renderJobSpecialSection(baseJob, total, p);
    ...
    content.innerHTML = `
      ...
      <div class="section">
        <div class="section-title">
          ...
          <span class="chip">${p.nickname || "(未命名)"} · ${p.job || "job?"} · Lv.${p.level}</span>
        </div>
      ...
      <div class="section">
        ...
        <div class="card">
          ...
          <div class="sep"></div>
          <div class="card-head"><span class="chip">來源</span></div>
