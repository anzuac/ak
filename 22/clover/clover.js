...
function initializeCloverSystem() {
  if (typeof player !== 'undefined' && player.coreBonus) {
    if (!player.coreBonus.bonusData) {
      player.coreBonus.bonusData = {};
    }
    // ====== 修正：確保幸運草數據被載入，如果沒有則初始化 ======
    if (!player.coreBonus.bonusData.clover) {
      player.coreBonus.bonusData.clover = { level: 0, expBonus: 0, dropBonus: 0, goldBonus: 0 };
    }
    // ====================================================================
    return;
  }
...
// ====== 修正：確保扣除資源後呼叫存檔 ======
function spendGold(n)    { player.gold  = getGold()    - n; updateResourceUI?.(); saveGame?.(); }
function spendDiamond(n) { player.gem   = getDiamond() - n; updateResourceUI?.(); saveGame?.(); }
function spendStone(n)   { player.stone = getStone()   - n; updateResourceUI?.(); saveGame?.(); }
// ===========================================
...
function renderCloverModal() {
  ...
  // ====== 修正：從 player 物件中讀取幸運草數據 ======
  const cloverData = player.coreBonus?.bonusData?.clover || { level: 0, expBonus: 0, dropBonus: 0, goldBonus: 0 };
...
// 修正：將 applyGainAndLog 改為直接操作 player 物件，並在最後觸發存檔
function applyGainAndLog(routeLabel) {
  // 確保 player.coreBonus.bonusData.clover 存在
  if (!player.coreBonus?.bonusData?.clover) {
    player.coreBonus.bonusData.clover = { level: 0, expBonus: 0, dropBonus: 0, goldBonus: 0 };
  }

  const cloverData = player.coreBonus.bonusData.clover;
  cloverData.level++;
