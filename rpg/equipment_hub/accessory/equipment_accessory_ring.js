...
(function(w){
  ...
  var ACCESSORY_LIST = [ACCESSORIES.ringSpeed, ACCESSORIES.ringLife, ACCESSORIES.ringBrute, ACCESSORIES.ringTreasure];

  // ===== 強化/星力/突破（通用參數；每顆戒指獨立持有等級與 Pity）=====
  var ENH = {
    baseSuccess: 0.20,       // 20%
    failBonus:   0.03,       // 失敗 +3%
  ...
  // ★混合強化規則：flat 走平坦、percent 走百分比（可調手感）
  var ENH_RULES = {
    // 每級 + 固定數字
  ...
  // ===== 存檔（獨立狀態）=====
  var LS_KEY = "ACC_MULTI_RING_INDEPENDENT_V1";
  var state = (function load(){
    ...
    function ensureRing(s, id){
      s.rings = s.rings || {};
      if (!s.rings[id]) s.rings[id] = freshRingState();
    }
    function sanitize(s){
      for (var i=0;i<ACCESSORY_LIST.length;i++){
        ensureRing(s, ACCESSORY_LIST[i].id);
      }
      for (var rid in s.rings){
        if (!s.rings.hasOwnProperty(rid)) continue;
        var r = s.rings[rid] || {};
  ...
  function doEnhance(id){
    ...
    if (roll(suc)){ r.enhLv+=1; r.enhPity=0; saveLocal(); applyBonusToPlayer(); return { ok:true, success:true, lv:r.enhLv, chance:suc }; }
    r.enhPity+=1; saveLocal(); return { ok:true, success:false, lv:r.enhLv, chance:suc };
  }
  function doStar(id){
    var r = state.rings[id];
    ...
    if (roll(suc)){ r.starLv+=1; r.starPity=0; saveLocal(); applyBonusToPlayer(); return { ok:true, success:true, lv:r.starLv, chance:suc }; }
    r.starPity+=1; saveLocal(); return { ok:true, success:false, lv:r.starLv, chance:suc };
  }
  function doBreak(id){
    var r = state.rings[id];
    ...
    if (roll(BREAK.success)){
      ...
      saveLocal(); return { ok:true, success:true, breakCount:r.breakCount, capEnh:r.capEnh, capStar:r.capStar };
    }
    saveLocal(); return { ok:true, success:false };
  }
