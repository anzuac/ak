...
(function (w) {
  ...
  // ===== 設定 =====
  const STORAGE_KEY = "sp.v1";            // 舊 localStorage key（僅用於遷移）
  const SAVEHUB_NS  = "sp.v1";     // SaveHub 命名空間
  ...
  // ===== SaveHub 封裝（統一存檔；localStorage 做後備 + 遷移）=====
  ...
  function normalizeSP(obj){
    ...
    try{
      ...
      base.percents= Object.assign({}, base.percents, obj.percents||{});
    }catch(_){}
    return base;
  }
  ...
  (function registerSaveHub(){
    ...
    try{
      var schema = {
        version: 1,
        migrate: function(old){ return normalizeSP(old||{}); }
      };
  ...
  // ===== 存檔 / 載入（SaveHub 優先；localStorage 後備 + 遷移）=====
  function saveLocal() {
  ...
  function loadLocal() {
    try {
      if (SH){
        // 先讀 SaveHub，若沒有再嘗試遷移舊 localStorage
        ...
        if (!data){
          ...
          if (raw){
            try{
              ...
              shWrite(data);
              localStorage.removeItem(STORAGE_KEY); // 遷移完成刪除舊檔
            }catch(_){ data = null; }
          }
        }
  ...
  function renderPercentRow(key) {
    ...
    if (el) el.textContent = `${Number(SP.percents[key]||0)} / ${PERCENT_LIMITS[key]}`;
  }
  ...
  function refreshInlineCP() {
    ...
    if (typeof w.getDisplayedCombatPower === "function") {
      try { cp = w.getDisplayedCombatPower(); } catch(_) {}
    }
    if (cp == null && typeof w.computeCombatPower === "function") {
      try { cp = w.computeCombatPower(); } catch(_) {}
    }
    if (cp == null) {
      const node =
  ...
  // ===== 初始化 =====
  function init() {
    loadLocal();           // ← SaveHub 優先；無則遷移 localStorage
    ensureModal();
    applyWhenReady();
  }
