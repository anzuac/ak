...
function drawUntilTarget() {
  ...
  const t3 = document.getElementById("target3").value;
  
  const invalidFirstOptions = getInvalidFirstOptions();
  if (invalidFirstOptions.includes(t1)) {
  ...
  const match1 = (t1 === "請選擇目標詞條 1" || r1 === t1);
  const match2 = (t2 === "請選擇目標詞條 2" || r2 === t2);
  const match3 = (t3 === "請選擇目標詞條 3" || r3 === t3);
...
function updateTargetOptionsByTier() {
  ...
  for (let select of selects) {
    select.innerHTML = '<option>請選擇目標詞條 ' + select.id.slice(-1) + '</option>';
    for (let opt of pool) {
