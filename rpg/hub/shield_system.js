// =======================
// shield_system.js — 護盾系統 V4（集中設定＋護盾值也存檔）
...
// 依賴：player、GrowthHub、updateResourceUI/saveGame、getItemQuantity/removeItem
// =======================
(function(w){
  "use strict";
  ...
  var AUTO_INTERVAL_MAX  = 60000;// 60s（防止誤設超大）
  ...
  function applyToPlayer(){
    ...
    var bag = player.coreBonus.bonusData.shieldSys = {};
    ...
    // 上限與護盾值（用本模組的存檔為準）
    player.maxShield = cap;
    var fromSave = Math.max(0, Number(S.shieldValue)||0);
    player.shield = Math.min(fromSave, cap);

    try{ w.updateResourceUI?.(); w.saveGame?.(); }catch(_){}
  }
  ...
  setInterval(function(){
    if (!w.player) return;
    var cap = maxShield();
    var cur = Math.max(0, Math.min(Number(player.shield||0), cap));
    if (cur !== _lastSnap){
      _lastSnap = cur;
      S.shieldValue = cur;
      save();
  ...
  function unlock(){
    ...
    if ((player.gold||0) < UNLOCK_COST_GOLD){ alert("金幣不足，需要 " + fmt(UNLOCK_COST_GOLD)); return; }
    player.gold -= UNLOCK_COST_GOLD;
    S.unlocked = true;
    // 初次解鎖不自動補；維持存檔值
    save(); applyToPlayer();
    alert("已解鎖護盾系統！");
  ...
  function upgradeGold(){
    if (!S.unlocked){ alert("尚未解鎖"); return; }
    if (!canRaiseTo("gold")){ alert("金幣升級已達上限（"+LV_CAP_GENERAL+"）"); return; }
    var cost = GOLD_COST_FOR_NEXT(S.level);
    if ((player.gold||0) < cost){ alert("金幣不足，需要 " + fmt(cost)); return; }
    player.gold -= cost;
    S.level += 1; save(); applyToPlayer();
  }
  function upgradeTicket(){
    if (!S.unlocked){ alert("尚未解鎖"); return; }
    if (!canRaiseTo("ticket")){ alert("等級已達上限（"+LV_CAP_TICKET+"）"); return; }
    if (invQty(ITEM_TICKET) < 1){ alert("缺少道具："+ITEM_TICKET); return; }
    invRemove(ITEM_TICKET,1);
    S.level += 1; save(); applyToPlayer();
  }
  function extendCap(){
    if (!S.unlocked){ alert("尚未解鎖"); return; }
    if ((S.capBoostSteps|0) >= CAP_STEPS_MAX){ alert("上限倍率已達 "+CAP_MULT_MAX.toFixed(2)+"×"); return; }
    if (invQty(ITEM_CAPSTONE) < 1){ alert("缺少道具："+ITEM_CAPSTONE); return; }
    invRemove(ITEM_CAPSTONE,1);
  ...
  function refill(isManual){
    ...
    if (!chk.ok){
      if (isManual){
        if (chk.reason==="over_threshold") alert("護盾尚未低於門檻");
        else if (chk.reason==="no_item") alert("缺少道具："+ITEM_REFILL);
        else if (chk.reason==="interval") alert("補充過快，請稍後再試");
        else if (chk.reason==="locked") alert("尚未解鎖護盾系統");
      }
      return false;
    }
    if (!invRemove(ITEM_REFILL,1)){ if(isManual) alert("道具扣除失敗"); return false; }
  ...
  function render(container){
    ...
    // 綁定
    if (!S.unlocked){
      var u = container.querySelector('#btnUnlock'); if (u) u.onclick = function(){ unlock(); w.GrowthHub.requestRerender(); };
      return;
    }
    var g = container.querySelector('#btnGoldUp');   if (g) g.onclick = function(){ upgradeGold();   w.GrowthHub.requestRerender(); };
    var t = container.querySelector('#btnTicketUp'); if (t) t.onclick = function(){ upgradeTicket(); w.GrowthHub.requestRerender(); };

    var cap = container.querySelector('#btnCapUp');  if (cap) cap.onclick = function(){ extendCap(); w.GrowthHub.requestRerender(); };
    ...
    var rf = container.querySelector('#btnRefill'); if (rf) rf.onclick = function(){ refill(true); w.GrowthHub.requestRerender(); };
  }
