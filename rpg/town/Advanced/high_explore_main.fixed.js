...
(function (w) {
  "use strict";
  // UI refresh guard for this tab only
  var __hex_last_container = null;
  function __hex_rerender(){ if (__hex_last_container){ try{ render(__hex_last_container); }catch(_){} } }
  ...
  function finishRun(slot){
    ...
    try { drops = computeDropsForDiff(diffId) || []; } catch (e) { console.error("[HighExplore] drop error:", e); }
    ...
    var s=(drops&&drops.length)? drops.map(function(d){
      var icon=(d.type==='gem'?'💎':(d.type==='gold'?'🪙':'📦'));
      return icon + d.key + '×' + d.qty;
    }).join('、') : '未獲得任何物品';
    var logLine = time+' 取得：'+s+'（'+(diff?diff.name:line.diffId)+'）';
...
// ===== UI =====
  ...
  function render(container){
    __hex_last_container = container;
    ensureRefill();
    ...
    var nextInfo = '';
    if (nextDiff){
      nextInfo = nextDiff.reqRank
        ? ('下一檔：'+nextDiff.name+'｜需求段位：<b>'+nextDiff.reqRank+'</b>')
        : ('下一檔：'+nextDiff.name+'｜需求CP：<b>'+fmt(nextDiff.reqCP||0)+'</b>');
    }else{
      nextInfo = '已達最高難度';
    }
    ...
    for(var i=0;i<btns.length;i++){
      btns[i].onclick=function(){
        ...
        if (!ok){
          // 補充原因提示（最佳努力）
          if (!hasChargeOrTicket()) showToast('⚠️ 沒有免費次數或 '+TICKET_NAME, true);
        }else{
          showToast('已開始探索');
        }
        w.__hex_rerender();
    ...
    if(bu){
      bu.onclick=function(){
        ...
        if(gem<SLOT_UNLOCK_COST){ showToast('⚠️ 您的資源不足：鑽石不足，需要 '+fmt(SLOT_UNLOCK_COST), true); return; }
        w.player.gem=Math.max(0,gem-SLOT_UNLOCK_COST);
        state.slots.push(newSlot(state.slots.length));
        persist();upd();saveGame();showToast('已解鎖新探索槽');
        w.__hex_rerender();
      };
    }
  }
