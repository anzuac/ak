...
(function(){
  ...
  function normalizeState(o){
    ...
    var out = {
      auto: (s.auto && typeof s.auto === 'object') ? s.auto : {},
      cooldowns: (s.cooldowns && typeof s.cooldowns === 'object') ? s.cooldowns : {},
      manualTs: Number(s.manualTs||0)
    };
    return out;
  }
  ...
  (function registerSaveHub(){
    ...
    }catch(e){ console && console.warn && console.warn("[potions_tab] SaveHub register failed:", e); }
  })();
  ...
  function shGet(defVal){
    ...
    }catch(e){ console && console.warn && console.warn("[potions_tab] SaveHub get failed:", e); }
    return defVal;
  }
  function shSet(val){
    if (!SH) return;
    ...
    }catch(e){ console && console.warn && console.warn("[potions_tab] SaveHub set failed:", e); }
  }
  ...
  function loadState(){
    try {
      if (SH){
        var data = shGet(null);
        if (!data){
          // 無 SaveHub，嘗試從舊 localStorage 遷移
          var raw = localStorage.getItem(SAVE_KEY);
          if (raw){
            try{
              var legacy = JSON.parse(raw);
              data = normalizeState(legacy);
              shSet(data);
              localStorage.removeItem(SAVE_KEY); // 遷移完成，移除舊檔
            }catch(_){ data = null; }
          }
        }
