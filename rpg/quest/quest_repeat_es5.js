...
(function(){
  ...
  // 可選：在 SaveHub 註冊版本（若你不需要流程化遷移，這段也可以不寫）
  ...
  function load(){
    try{
      if (!SH) return defState(); // 沒有 SaveHub 就用預設，避免腳本報錯
  ...
  var THRESH_MUL = 1.2;   // 難度提升
  var REWARD_MUL = 1.05;  // 獎勵提升（五捨六入）
