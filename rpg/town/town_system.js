...
(function (w) {
  ...
  var state = (function load(){
    try{
      ...
      o.bankProducedToday = o.bankProducedToday||0;
      o.mineProducedToday = o.mineProducedToday||0;
      o.campProducedToday = o.campProducedToday||0;
      o._prodCarrySec = o._prodCarrySec||0;
      o._campCarrySec = o._campCarrySec||0;
      o._lastDaily = o._lastDaily || dailyKey();
      return o;
    }catch(_){
      return {
        mainCityLv: 0, mainCityFeed: 0,
  ...
  function tryUpgradeBank(){
    ...
    if (g<cost){ notify("💰 金幣不足，需要 "+fmt(cost)); return; }
    w.player.gold = g - cost;
    state.bankUpStart = nowSec();
    saveLocal(); upd(); save(); notify("🏦 銀行開始升級（8 小時）");
  }
  function finishBank(){ var r=bankRemainSec(); if(r>0||!state.bankUpStart) return; state.bankUpStart=0; state.bankLv=clamp(state.bankLv+1,1,BANK_MAX); saveLocal(); notify("🏦 銀行升級完成 → Lv."+state.bankLv); }
  ...
  function tryUpgradeMine(){
    ...
    saveLocal(); upd(); save(); notify("⛏ 礦洞開始升級（8 小時）");
  }
  function finishMine(){ var r=mineRemainSec(); if(r>0||!state.mineUpStart) return; state.mineUpStart=0; state.mineLv=clamp(state.mineLv+1,1,MINE_MAX); saveLocal(); notify("⛏ 礦洞升級完成 → Lv."+state.mineLv); }
  ...
  function beginCampUpgrade(){
    ...
    if (gem<cost){ notify("💎 鑽石不足，需要 "+fmt(cost)); return; }
    w.player.gem = gem - cost;
    state.campUpStart = nowSec();
    saveLocal(); upd(); save(); notify("🏋️ 訓練營開始升級（"+CAMP_UPGRADE_WAIT_HOURS+" 小時）");
  }
  function finishCamp(){ var st=campStatus(); if(!st||st.remainSec>0) return; state.campUpStart=0; state.campLv=clamp(state.campLv+1,1,CAMP_MAX); saveLocal(); notify("🏋️ 訓練營升級完成 → Lv."+state.campLv); }
  ...
  function render(container){
    ...
    function sectionCard(title, inner){
      return '<div style="background:#191b25;border:1px solid #2f3555;border-radius:10px;padding:10px;margin-bottom:12px">'+
    ...
    var b50= container.querySelector('#btnFeed50'); if (b50) b50.onclick = function(){
      var lv = Math.max(1, state.mainCityLv || 1);
      var p0 = 0.01 / lv;
      var chance = 100 * (1 - Math.pow(1 - p0, 50));
      if (confirm("確定餵 50 顆精華？\n\n基礎單顆約 "+(p0*100).toFixed(2)+"%\n50 顆估算約 "+chance.toFixed(1)+"%")){
        feedEssence(50); w.TownHub.requestRerender();
      }
    };
