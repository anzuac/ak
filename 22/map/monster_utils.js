...
function getMonster(area, levelRange) {  
  // ========== Boss 遭遇（地圖王）==========  
  if (typeof mapBossPool !== 'undefined') {  
    ...
    for (const bossConfig of mapSpecificBosses) {  
      ...
      if (Number.isFinite(encounterChance) && Math.random() < encounterChance) {  
        if (typeof showBossEncounterUI === 'function') { try { showBossEncounterUI(bossConfig.name); } catch(_){ } }  
        if (typeof logPrepend === 'function') { try { logPrepend(`❗❗ 一股強大的氣息出現了...`); } catch(_){ } }  
  ...
  const difficulty = normalizeDifficulty(  
    typeof getCurrentDifficulty === "function" ? getCurrentDifficulty() : {}  
  );  
  ...
  const pool = areaData?.includeAll  
    ? [...(Array.isArray(monsterBasePool) ? monsterBasePool : []), ...(Array.isArray(areaData?.monsters) ? areaData.monsters : [])]  
    : (Array.isArray(areaData?.monsters) && areaData.monsters.length ? areaData.monsters : (Array.isArray(monsterBasePool) ? monsterBasePool : []));  
